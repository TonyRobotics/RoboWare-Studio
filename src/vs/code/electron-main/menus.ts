...
export class VSCodeMenu {
	...
	private install(): void {
		...
		// Ros
		let rosMenu = new Menu();
		let rosMenuItem = new MenuItem({ label: mnemonicLabel(nls.localize({ key: 'mRos', comment: ['&& denotes a mnemonic'] }, "&&ROS")), submenu: rosMenu });
		this.setRosMenu(rosMenu);
	...
	private setFileMenu(fileMenu: Electron.Menu): void {
		...
		let newWorkspace = this.createMenuItem(nls.localize({ key: 'miNewWorkspace', comment: ['&& denotes a mnemonic'] }, "New &&Workspace..."), 'workbench.action.files.newWorkspace', this.windowsService.getWindowCount() > 0);
		...
		const openFolder = new MenuItem(this.likeAction('workbench.action.files.openFolder', { label: mnemonicLabel(nls.localize({ key: 'miOpenFolder', comment: ['&& denotes a mnemonic'] }, "Open &&Workspace...")), click: (menuItem, win, event) => this.windowsService.openFolderPicker(this.isOptionClick(event), undefined, { from: telemetryFrom }) }));
		...
		const closeFolder = this.createMenuItem(nls.localize({ key: 'miCloseFolder', comment: ['&& denotes a mnemonic'] }, "Close &&Workspace"), 'workbench.action.closeFolder');
	...
	private getPreferencesMenu(): Electron.MenuItem {
		const languageSettings = this.createMenuItem(nls.localize({ key: 'miOpenLanguageSettings', comment: ['&& denotes a mnemonic'] }, "&&Language Settings"), 'workbench.action.configureLocale');
		...
		preferencesMenu.append(languageSettings);
	...
	private setEditMenu(winLinuxEditMenu: Electron.Menu): void {
		...
		const toggleVim = this.createMenuItem(nls.localize({ key: 'miToggleVimMode', comment: ['&& denotes a mnemonic'] }, "Toggle &&Vim Mode"), 'workbench.extensions.action.toggleVimModeAction');
		[
			undo,
			redo,
			__separator__(),
	...
	private setViewMenu(viewMenu: Electron.Menu): void {
		...
		const ros = this.createMenuItem(nls.localize({ key: 'miViewRos', comment: ['&& denotes a mnemonic'] }, "&&ROS"), 'workbench.view.ros');
	...
	private setRosMenu(rosMenu: Electron.Menu): void {
		const build = this.createMenuItem(nls.localize({ key: 'miBuild', comment: ['&& denotes a mnemonic'] }, "&&Build"), 'workbench.action.tasks.build');
		const bashrc = this.createMenuItem(nls.localize({ key: 'miBashrc', comment: ['&& denotes a mnemonic'] }, "&&Open ~/.bashrc"), 'workbench.action.ros.openBashrc');
		const roscore = this.createMenuItem(nls.localize({ key: 'miRoscore', comment: ['&& denotes a mnemonic'] }, "Run ros&&core"), 'workbench.action.ros.runRosCore');
		const rviz = this.createMenuItem(nls.localize({ key: 'miRviz', comment: ['&& denotes a mnemonic'] }, "Run &&RViz"), 'workbench.action.ros.runRosRviz');
		const rqt = this.createMenuItem(nls.localize({ key: 'miRqt', comment: ['&& denotes a mnemonic'] }, "Run r&&qt"), 'workbench.action.ros.runRosRqt');
		const reconfigure = this.createMenuItem(nls.localize({ key: 'miReconfigure', comment: ['&& denotes a mnemonic'] }, "Run rqt-recon&&figure"), 'workbench.action.ros.runRosReconfigure');
		const graph = this.createMenuItem(nls.localize({ key: 'miGraph', comment: ['&& denotes a mnemonic'] }, "Run rqt-&&graph"), 'workbench.action.ros.runRosGraph');
		const remoteBashrc = this.createMenuItem(nls.localize({ key: 'miRemoteBashrc', comment: ['&& denotes a mnemonic'] }, "&&Open Remote ~/.bashrc"), 'workbench.action.ros.openRemoteBashrc');
		const remoteRoscore = this.createMenuItem(nls.localize({ key: 'miRemoteRoscore', comment: ['&& denotes a mnemonic'] }, "Run Remote ros&&core"), 'workbench.action.ros.runRemoteRosCore');
		...
		[
			...
			remoteBashrc,
			remoteRoscore
		].forEach(item => rosMenu.append(item));
	}
