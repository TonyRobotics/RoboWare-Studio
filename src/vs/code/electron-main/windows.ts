...
import { rosLaunchJson } from 'vs/workbench/services/ros/node/rosService';
	...
	public open(openConfig: IOpenConfiguration): VSCodeWindow[] {
		...
		if (openConfig.pathsToOpen && openConfig.pathsToOpen.length > 0) {
			iPathsToOpen = openConfig.pathsToOpen.map(pathToOpen => {
				...
				if (!iPath) {
				...
				} else if (iPath.workspacePath) {

					// Popup warning when the folder to open is not a ROS workspace
					const cmlPath = path.join(iPath.workspacePath, 'src', 'CMakeLists.txt');
					try {
						fs.readlinkSync(cmlPath);
					} catch (error) {
						const options: Electron.ShowMessageBoxOptions = {
							title: product.nameLong,
							type: 'info',
							buttons: [nls.localize('ok', "OK")],
							message: nls.localize('pathNotWorkspaceTitle', "Path is not ROS Workspace"),
							detail: nls.localize('pathNotWorkspaceDetail', "The path '{0}' does not seem to be an effective ROS Workspace on disk.", pathToOpen),
							noLink: true
						};
					...
					// Check and restore the initial configuration file and the necessary directories
					const cfgPath = path.join(iPath.workspacePath, '.vscode');
					if (!fs.existsSync(cfgPath) || !fs.lstatSync(cfgPath).isDirectory) {
						fs.mkdirSync(cfgPath);
					}
					const taskPath = path.join(cfgPath, 'tasks.json');
					if (!fs.existsSync(taskPath) || !fs.lstatSync(taskPath).isFile) {
						fs.writeFileSync(taskPath, taskTemplates[0].content);
					}
					const launchPath = path.join(cfgPath, 'launch.json');
					if (!fs.existsSync(launchPath) || !fs.lstatSync(launchPath).isFile) {
						fs.writeFileSync(launchPath, rosLaunchJson);
					}
					const develPath = path.join(iPath.workspacePath, 'devel');
					if (!fs.existsSync(develPath) || !fs.lstatSync(develPath).isDirectory) {
						fs.mkdirSync(develPath);
					}
					const libPath = path.join(develPath, 'lib');
					if (!fs.existsSync(libPath) || !fs.lstatSync(libPath).isDirectory) {
						fs.mkdirSync(libPath);
					}
					const isolatedPath = path.join(iPath.workspacePath, 'devel_isolated');
					if (!fs.existsSync(isolatedPath) || !fs.lstatSync(isolatedPath).isDirectory) {
						fs.mkdirSync(isolatedPath);
					}
					const remoteDevelPath = path.join(iPath.workspacePath, 'el');
					if (!fs.existsSync(remoteDevelPath) || !fs.lstatSync(remoteDevelPath).isDirectory) {
						fs.mkdirSync(remoteDevelPath);
					}
					const remoteLibPath = path.join(remoteDevelPath, 'lib');
					if (!fs.existsSync(remoteLibPath) || !fs.lstatSync(remoteLibPath).isDirectory) {
						fs.mkdirSync(remoteLibPath);
					}
					const remoteIsolatedPath = path.join(iPath.workspacePath, 'el_isolated');
					if (!fs.existsSync(remoteIsolatedPath) || !fs.lstatSync(remoteIsolatedPath).isDirectory) {
						fs.mkdirSync(remoteIsolatedPath);
					}
					const ycmConfPath = path.join(iPath.workspacePath, '.ycm_extra_conf.py');
					if (!fs.existsSync(ycmConfPath) || !fs.lstatSync(ycmConfPath).isFile) {
						const wsdevinc = `'-isystem',\n'${path.join(iPath.workspacePath, 'devel', 'include')}',\n`;
						const srcDir = path.join(iPath.workspacePath, 'src');
						const children = fs.readdirSync(srcDir);
						const wspkginc = children.map(val => `'-isystem',\n'${path.join(srcDir, val, 'include')}'`).join(',\n');
						fs.writeFileSync(ycmConfPath, rosTemplateYcmConf.replace('${ws_incs}', wsdevinc + wspkginc));
					}
				}
