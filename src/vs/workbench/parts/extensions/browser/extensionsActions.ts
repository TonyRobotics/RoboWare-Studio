...
export class ToggleVimAction extends Action {

	static ID = 'workbench.extensions.action.toggleVimModeAction';
	static LABEL = localize('toggleVimModeAction', "Toggle Vim Mode");
	...
	run(): TPromise<any> {
		const vimModeDisabled = this.extensionEnablementService.getGloballyDisabledExtensions().indexOf('vscodevim.vim') >= 0;
		return this.extensionEnablementService.setEnablement('vscodevim.vim', vimModeDisabled, false).then(() => {
			if (this.messageService.confirm({ message: localize('vimMessage', "Reload this window to make the changes take effect."), primaryButton: localize('reload', "&&Reload Window") })) {
				return this.windowService.reloadWindow();
			}
			return null;
		});
