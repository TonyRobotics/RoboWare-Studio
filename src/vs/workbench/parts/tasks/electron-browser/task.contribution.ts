				...
				return this._taskSystem.terminateAll().then((response) => {
					if (response.success) {
						...
						return !this.messageService.confirm({
							message: nls.localize('TaskSystem.noProcess', 'The launched task doesn\'t exist anymore. If the task spawned background processes exiting RoboWare Studio might result in orphaned processes. To avoid this start the last background process with a wait flag.'),
