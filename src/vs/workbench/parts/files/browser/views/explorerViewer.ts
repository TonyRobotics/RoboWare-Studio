...
import { FileStat, NewStatPlaceholder, StatType } from 'vs/workbench/parts/files/common/explorerViewModel';
...
export class FileRenderer implements IRenderer {
	...
	public renderElement(tree: ITree, stat: FileStat, templateId: string, templateData: IFileTemplateData): void {
		...
		if (!editableData) {
			...
			let extraClasses = ['explorer-item'];
			if (this.isRosRootSrcFolder(stat)) {	// 预读取活动包名
				this.activePkgName = this.rosService.getActivePkgNameCacheSync();
			} else if (this.isRosPackageFolder(stat)) {	// 非活动包设置样式
				if (this.activePkgName && this.activePkgName.indexOf(stat.name) < 0) {
					extraClasses.push('inactive-item');
				}
			}
...
export class FileFilter implements IFilter {
	...
	private doIsVisible(stat: FileStat): boolean {
		...
		const relativePath = this.contextService.toWorkspaceRelativePath(stat.resource, true);
		const siblingsFn = () => siblings && siblings.map(c => c.name);
		if (glob.match(this.hiddenExpression, relativePath, siblingsFn)) {
