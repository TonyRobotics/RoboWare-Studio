...
const DEFAULT_ICON_THEME_ID = 'vscode.theme-defaults-vs-minimal';
...
export class WorkbenchThemeService implements IWorkbenchThemeService {
		...
		return this.backupSettings().then(backupLocation => {
			...
			if (!types.isUndefined(legacyIconThemeId)) {
				...
				promise = promise.then(_ => {
					return this._findIconThemeData(legacyIconThemeId, DEFAULT_ICON_THEME_ID).then(theme => {
						let value = theme ? theme.settingsId : DEFAULT_ICON_THEME_SETTING_VALUE;
	...
	private initialize(): TPromise<any> {
		...
		return Promise.join([
			...
			this.findIconThemeBySettingsId(iconThemeSetting, DEFAULT_ICON_THEME_ID).then(theme => {
	...
	private installConfigurationListener() {
		this.configurationService.onDidUpdateConfiguration(e => {
			...
			if (iconThemeSetting !== this.currentIconTheme.settingsId) {
				this.findIconThemeBySettingsId(iconThemeSetting, null).then(theme => {
	...
	public setFileIconTheme(iconTheme: string, settingsTarget: ConfigurationTarget): TPromise<IFileIconTheme> {
		...
		return this._findIconThemeData(iconTheme, DEFAULT_ICON_THEME_ID).then(iconThemeData => {
	...
	private _findIconThemeData(iconTheme: string, defaultId: string): TPromise<IInternalIconThemeData> {
		return this.getFileIconThemes().then(allIconSets => {
			let defaultIconTheme: IInternalIconThemeData = null;
			for (let iconSet of allIconSets) {
				if (iconSet.id === iconTheme) {
					return <IInternalIconThemeData>iconSet;
				}
				if (iconSet.id === defaultId) {
					defaultIconTheme = <IInternalIconThemeData>iconSet;
				}
			}
	...
	private findIconThemeBySettingsId(settingsId: string, defaultId: string): TPromise<IFileIconTheme> {
		return this.getFileIconThemes().then(allIconSets => {
			let defaultIconTheme: IFileIconTheme = null;
			for (let iconSet of allIconSets) {
				if (iconSet.settingsId === settingsId) {
					return iconSet;
				}
				if (iconSet.id === defaultId) {
					defaultIconTheme = iconSet;
				}
			}
			return defaultIconTheme;
		});
