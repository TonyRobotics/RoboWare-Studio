...
export class ExtensionManagementService implements IExtensionManagementService {
	...
	install(zipPath: string): TPromise<void> {
		...
		return validate(zipPath).then<void>(manifest => {
			...
			return this.isObsolete(id).then(isObsolete => {
				if (isObsolete) {
					return TPromise.wrapError(new Error(nls.localize('restartCode', "Please restart RoboWare Studio before reinstalling {0}.", manifest.displayName || manifest.name)));
				}
	...
	installFromGallery(extension: IGalleryExtension, promptToInstallDependencies: boolean = true): TPromise<void> {
		...
		return this.isObsolete(id).then(isObsolete => {
			if (isObsolete) {
				return TPromise.wrapError<void>(new Error(nls.localize('restartCode', "Please restart RoboWare Studio before reinstalling {0}.", extension.displayName || extension.name)));
			}
	...
	private installWithDependencies(extension: IGalleryExtension): TPromise<ILocalExtension> {
		return this.galleryService.getAllDependencies(extension)
			...
			.then(toInstall => this.filterObsolete(...toInstall.map(i => getLocalExtensionIdFromGallery(i, i.version)))
				.then((obsolete) => {
					if (obsolete.length) {
						return TPromise.wrapError<ILocalExtension>(new Error(nls.localize('restartCode', "Please restart RoboWare Studio before reinstalling {0}.", extension.displayName || extension.name)));
					}
