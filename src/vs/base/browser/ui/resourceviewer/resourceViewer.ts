...
export class ResourceViewer {
	...
	public static show(
		...
		// Handle generic Binary Files
			...
			if (commandService && ext.toLowerCase() === '') {
				let titleDiv = $('div.section').appendTo(container);
				$('p').text(nls.localize('debugFileHelp', "You can debug this binary file.")).appendTo(titleDiv);

				let section = $('div.section').appendTo(container);

				let button = new Button(section);
				button.label = nls.localize('debugFile', "Debug this file");
				button.addListener('click', () => {
					return commandService.executeCommand('workbench.action.debug.start');
				});

				let titleDiv2 = $('div.section').appendTo(container);
				$('p').text(nls.localize('runFileHelp', "You can also run this binary file.")).appendTo(titleDiv2);

				let section2 = $('div.section').appendTo(container);

				let button2 = new Button(section2);
				button2.label = nls.localize('runFile', "Run this file");
				button2.addListener('click', () => {
					return commandService.executeCommand('workbench.action.debug.run');
				});

				let titleDiv3 = $('div.section').appendTo(container);
				$('p').text(nls.localize('configArgsHelp', "You may need to configure the command args when debugging or running.")).appendTo(titleDiv3);

				let section3 = $('div.section').appendTo(container);

				let button3 = new Button(section3);
				button3.label = nls.localize('configArgs', "Configure Args");
				button3.addListener('click', () => {
					return commandService.executeCommand('workbench.action.ros.configLaunchArgs');
				});

				let titleDiv4 = $('div.section').appendTo(container);
				$('p').text(nls.localize('selectArgsHelp', "Select the args that have been used before.")).appendTo(titleDiv4);

				let section4 = $('div.section').appendTo(container);

				let button4 = new Button(section4);
				button4.label = nls.localize('selectArgs', "Select Args");
				button4.addListener('click', () => {
					return commandService.executeCommand('workbench.action.ros.selectLaunchArgs');
				});
			}
		}
	}
