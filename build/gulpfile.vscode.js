...
function renameArch(pkgName, path, name, arch) {
	const source = arch === 'x64' ?
		'node_modules/' + pkgName + '/build/Release/' + path + name :
		'nm_arch/' + pkgName + '/' + arch + '/' + name;
	return gulp.src(source)
		.pipe(rename('node_modules/' + pkgName + '/build/Release/' + path + name));
}

function renameArch2(pkgName, name, arch) {
	const source = arch === 'x64' ?
		'node_modules/' + pkgName + '/bin/' + name :
		'nm_arch/' + pkgName + '/' + arch + '/' + name;
	return gulp.src(source)
		.pipe(rename('node_modules/' + pkgName + '/bin/' + name));
}
...
function packageTask(platform, arch, opts) {
	...
	return () => {
		...
		const deps = gulp.src(depsSrc, { base: '.', dot: true })
			...
			.pipe(util.cleanNodeModule('oniguruma', ['binding.gyp', 'build/**', 'src/**', 'deps/**']))	// delete includes args: ['**/*.node']
			.pipe(util.cleanNodeModule('windows-mutex', ['binding.gyp', 'build/**', 'src/**'], ['**/*.node']))
			.pipe(util.cleanNodeModule('native-keymap', ['binding.gyp', 'build/**', 'src/**', 'deps/**']))	// delete includes args: ['**/*.node']
			.pipe(util.cleanNodeModule('jschardet', ['dist/**']))
			.pipe(util.cleanNodeModule('windows-foreground-love', ['binding.gyp', 'build/**', 'src/**'], ['**/*.node']))
			.pipe(util.cleanNodeModule('gc-signals', ['binding.gyp', 'build/**', 'src/**', 'deps/**'], ['src/index.js']))	// delete includes args: '**/*.node'
			.pipe(util.cleanNodeModule('v8-profiler', ['binding.gyp', 'build/**', 'src/**', 'deps/**'], ['src/index.js']))	// delete includes args: '**/*.node'
			.pipe(util.cleanNodeModule('node-pty', ['binding.gyp', 'build/**', 'src/**', 'tools/**']))	// delete includes args: ['build/Release/**']
			.pipe(util.cleanNodeModule('vscode-ripgrep', ['bin/**']));
		...
		// Replace the packages that are architecturally dependent
		if (platform === 'linux') {
			all = es.merge(all,
				renameArch('oniguruma', '', 'onig_scanner.node', arch),
				renameArch('oniguruma', 'obj.target/', 'onig_scanner.node', arch),
				renameArch('native-keymap', '', 'keymapping.node', arch),
				renameArch('native-keymap', 'obj.target/', 'keymapping.node', arch),
				renameArch('gc-signals', '', 'gcsignals.node', arch),
				renameArch('gc-signals', 'obj.target/', 'gcsignals.node', arch),
				renameArch('v8-profiler', '', 'profiler.node', arch),
				renameArch('v8-profiler', 'obj.target/', 'profiler.node', arch),
				renameArch('node-pty', '', 'pty.node', arch),
				renameArch2('vscode-ripgrep', 'rg', arch)
			);
		}
